...
contract MLP_3L_4n1n {
	...
	constructor(uint256 layer_num) {
        ...
        function set_Weights(uint256 layer, int256[] calldata w) external {
          ...
          else if (layer == 2) {              weights_layer3.push(temp_w);
          }
        }
...
SD59x18[] memory neuronResultsLayer3 = new SD59x18[](weights_layer3.length);
               for (uint256 n = 0; n < weights_layer3.length; n++) {
                ...
                for (uint256 i = 0; i < weights_layer2.length; i++) {
                  neuronResultsLayer3[n] = neuronResultsLayer3[n].add(neuronResultsLayer2[i].mul(SD59x18.wrap(weights_layer3[n][i])));
                }
              neuronResultsLayer3[n] = sigmoid(neuronResultsLayer3[n]);
		}


              int256 classification;
              SD59x18 point_five = sd(0.5e18);
              if (neuronResultsLayer3[0].gte(point_five)) {
